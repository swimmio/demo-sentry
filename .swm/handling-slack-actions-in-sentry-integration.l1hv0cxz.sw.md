...
---
This document will explain the flow of handling Slack actions in the Sentry integration. The flow includes the following steps:
...
The function `_handle_group_actions` is responsible for handling various Slack actions related to a group. It first retrieves the group and the identity of the acting user. Depending on the type of action, it performs different tasks such as updating the status, assigning the issue, or opening a resolve dialog. If any of these actions fail, it calls the `api_error` function to handle the error.
...
The function `api_error` is used to handle any API errors that occur during the handling of Slack actions. It logs the error and responds with an appropriate message to the user. If the error status code is 403, it provides a link to unlink the identity, otherwise, it sends a default error message.
...
The function `get_logging_data` is used to gather data for logging. It includes data from the Slack request and, if provided, additional data related to a group.
